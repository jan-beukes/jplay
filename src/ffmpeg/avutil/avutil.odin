...
*/
    ...
    hwframe_ctx_create_derived :: proc(derived_frame_ctx: ^^types.Buffer_Ref, format: types.Pixel_Format, derived_device_ctx: ^types.Buffer_Ref, source_frame_ctx: ^types.Buffer_Ref, flags: types.Hardware_Frame_Map_Flags) -> i32 ---
    ...
    dynarray2_add :: proc(tab_ptr: ^rawptr, nb_ptr: ^i32, elem_size: uintptr, elem_data: [^]u8) -> rawptr ---
    ...
    thread_message_queue_set_free_func :: proc(mq: ^types.Thread_Message_Queue, free_func: proc(_: ^rawptr)) ---
    ...
    // Finds an element
    tree_find :: proc(root: ^types.Tree_Node, key: rawptr, cmp: proc(_: ^rawptr, _: ^rawptr) -> i32, next: [2]rawptr) -> rawptr ---

    // Inserts or removes an element
    tree_insert :: proc(rootp: ^^types.Tree_Node, key: rawptr, cmp: proc(_: ^rawptr, _: ^rawptr) -> i32, next: ^types.Tree_Node) -> rawptr ---

    // Destroys a tree
    tree_destroy :: proc(t: ^types.Tree_Node) ---

    // Applies enu(opaque, &elem) to all the elements in the tree in a given range
    tree_enumerate :: proc(t: ^types.Tree_Node, opaque: rawptr, cmp: proc(_: rawptr, _: rawptr) -> i32, enu: proc(_: rawptr, _: rawptr) -> i32) ---
